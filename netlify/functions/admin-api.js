...
async function listUsage() {
  ...
  for (let offset = 0; offset < 100000; offset += 1000) {
    const rows = await db(`analysis_usage?model=eq.gemini-3.1-pro-preview&select=id,account_id,model,input_tokens,answer_tokens,thinking_tokens,output_tokens,total_tokens,estimated_cost_usd,created_at&order=created_at.desc&limit=1000&offset=${offset}`) || [];
    all.push(...rows);
...
async function payload() {
  return { accounts: await listAccounts(), usage: await listUsage(), pricing: PRICING };
}
