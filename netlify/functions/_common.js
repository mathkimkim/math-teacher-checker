...
export async function getSessionAccount(event){
  ...
  const acc=await db(`accounts?id=eq.${encodeURIComponent(session.account_id)}&select=id,login_id,limit_count,used_count,active`);
  return acc?.[0]?{token,session,account:acc[0]}:null;
}
