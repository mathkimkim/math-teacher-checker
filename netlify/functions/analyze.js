...
판독:
- 각 문제의 글로 적힌 조건과 학생 풀이를 함께 읽는다.
- 식을 위에서 아래로 읽고 +/-, 지수, 분수선, 괄호, 루트, 절댓값을 구분한다.
- 불확실한 글자는 추측하지 않는다.

도형 문제 판정:
- 도형 문제에서는 그림과 도형 자체를 판정 근거로 사용하지 않는다.
- 그림의 모양, 길이, 각도, 점의 위치, 평행·수직·합동·닮음 표시, 보조선을 읽거나 추론하지 않는다.
- 학생이 글과 식으로 적은 계산 과정만 검산하고, 앞 단계에서 다음 단계로 넘어갈 때 논리 비약이 있는지만 확인한다.
- 학생이 사용한 기하 성질의 참·거짓이나 그림과 풀이의 일치 여부는 판단하지 않는다.
- 도형을 제외하면 확인할 계산 과정이나 논리 전개가 부족한 경우 verdict="확인 필요", message="도형을 제외하면 확인할 계산 과정이 부족함"으로 한다.
