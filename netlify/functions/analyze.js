...
export async function handler(event) {
  ...
  // 분석 요청이 시작되는 시점에 1장을 차감합니다. 이후 성공·실패와 관계없이 복구하지 않습니다.
  ...
  if (!updatedRows?.length) {
    return json(409, errorBody({ title: '사용량 갱신 충돌', reason: '동시에 여러 분석이 시작되어 사용량을 갱신하지 못했습니다.', solution: ['잠시 후 다시 시도해 주세요.'], code: 'QUOTA_CONFLICT', status: 409 }));
  }
  const account = { ...auth.account, used_count: nextUsed };
