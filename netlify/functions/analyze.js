...
import { getSessionAccount, db } from './_common.js';
...
10. message 안에 수식이 꼭 필요하면 해당 수식만 반드시 \( ... \)로 감싼다.
...
function getResponseTotalTokens(data) {
  const usage = data?.usage || {};
  const value = usage.total_tokens ?? usage.total ?? usage.total_token_count ?? 0;
  const tokens = Math.floor(Number(value) || 0);
  return tokens > 0 ? tokens : 0;
}

async function addTokensToAccount(accountId, tokenCount) {
  const amount = Math.max(0, Math.floor(Number(tokenCount) || 0));
  if (!accountId || !amount) return null;

  // 같은 아이디로 여러 분석이 동시에 완료되어도 토큰이 빠지지 않도록
  // 현재 값 확인 → 조건부 갱신을 최대 6회 재시도한다.
  for (let attempt = 0; attempt < 6; attempt += 1) {
    const rows = await db(`accounts?id=eq.${encodeURIComponent(accountId)}&select=total_tokens`);
    const current = Math.max(0, Number(rows?.[0]?.total_tokens || 0));
    const next = current + amount;
    const updated = await db(
      `accounts?id=eq.${encodeURIComponent(accountId)}&total_tokens=eq.${current}`,
      { method: 'PATCH', body: { total_tokens: next } }
    ).catch(() => []);

    if (updated?.length) return next;
  }

  throw new Error('누적 토큰을 갱신하지 못했습니다.');
}
...
export async function handler(event) {
  ...
  const isRecheck = payload.mode === 'recheck';
  const problemNumber = String(payload.problemNumber || '').trim();
  const confirmedExpression = normalizeLatexExpression(payload.confirmedExpression || '');
  const confirmedMessage = normalizeMathMessage(payload.confirmedMessage || '');

  if (isRecheck && !problemNumber) {
    return json(400, errorBody({ title: '재검토 정보 오류', reason: '다시 검토할 문제 번호가 없습니다.', solution: ['분석 결과에서 맞음 버튼을 다시 눌러 주세요.'], code: 'PROBLEM_NUMBER_REQUIRED', status: 400, account: auth.account }));
  }

  let account = auth.account;

  // 분석 장수는 새 사진 분석을 시작할 때만 1장 차감합니다.
  // 같은 사진에서 선생님이 판정을 수정해 재검토하는 호출은 추가 차감하지 않습니다.
  if (!isRecheck) {
...
[선생님 수정 반영 재검토 규칙]
- ${problemNumber}번 문제만 검토한다. 다른 문제는 반환하지 않는다.
- 이전에 오류라고 판단한 단계는 선생님이 맞는 단계로 확정했다. 그 단계를 다시 오류라고 판단하지 않는다.
- 확정된 단계 다음부터 풀이 마지막까지 계속 검토한다.
- 이후에 새로운 최초 오류가 있으면 verdict="틀림"으로 반환한다.
- 이후에 오류가 없으면 verdict="맞음"으로 반환한다.
- 반드시 problems 배열에 ${problemNumber}번 결과 하나만 반환한다.`
              : systemPrompt
          },
          {
            role: 'user',
            content: [
              {
                type: 'input_text',
                text: isRecheck
                  ? `${problemNumber}번에서 이전 AI가 오류로 지적한 단계는 선생님이 맞다고 확정했습니다.\n확정된 식: ${confirmedExpression || '(식 정보 없음)'}\n이전 설명: ${confirmedMessage || '(설명 없음)'}\n이 단계를 정상으로 인정하고, 같은 문제의 그 다음 풀이부터 마지막까지 다시 검산해 최종 JSON만 반환하세요.`
                  : '사진 속 풀이가 있는 문제를 번호별로 모두 분리해 각각 검산하고 최종 JSON만 반환하세요.'
              },
    ...
    // 새 분석과 재검토 모두 실제 API 사용 토큰을 현재 로그인 아이디에 누적합니다.
    const responseTokens = getResponseTotalTokens(data);
    if (responseTokens > 0) {
      try {
        const totalTokens = await addTokensToAccount(auth.account.id, responseTokens);
        if (totalTokens !== null) account = { ...account, total_tokens: totalTokens };
      } catch (tokenError) {
    ...
    return json(200, { problems, account });
  } catch (error) {
    clearTimeout(timeoutId);
