...
function ResultCard({ item }) {
  const problems = getProblems(item.result);
  if (!problems.length) return null;

  return (
    <div className="result problemResults">
      {problems.map((problem, index) => {
        const label = problem.number ? `${problem.number}번` : `문제 ${index + 1}`;
        const message = problem.verdict === '맞음'
          ? '맞음'
          : (problem.message || problem.verdict || '확인 필요');

        return (
          <div className="problemResult" key={`${problem.number}-${index}`}>
            <strong className="problemNumber">{label}</strong>
            <span className={`verdict ${getVerdictClass(problem)}`}>{message}</span>
          </div>
        );
      })}
    </div>
...
function App() {
  ...
  const allProblems = items.flatMap((x) => getProblems(x.result));
  const cleanCount = allProblems.filter((x) => x.verdict === '맞음').length;
  const reviewCount = allProblems.filter((x) => x.verdict && x.verdict !== '맞음').length;
    ...
    <main className="app">
      <section className="hero">
        <div>
          ...
          <h1>풀이체커</h1>
          <p className="sub">한 페이지를 올리면 문제 번호별로 나누어 맞음 또는 처음 틀린 위치만 보여줍니다.</p>
        </div>
      ...
      >
        ...
        <small>풀이 글씨를 유지하면서 전송 크기를 자동 최적화합니다.</small>
      </section>
      ...
      {items.some((x) => x.result) && (
        <section className="summary">
          <div><b>{allProblems.length}</b><span>전체 문제</span></div>
        ...
        {items.map((item, idx) => (
          <article className="card" key={item.id}>
            ...
            <div className="cardBody">
              ...
              {item.status === 'analyzing' && <p className="muted">AI가 문제별로 풀이를 확인하는 중...</p>}
