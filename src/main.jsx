...
function TopbarQuota({ account }) {
  ...
  const totalTokens = Number(account?.total_tokens || 0);

  return (
    <div
      className="topbarQuota"
      aria-label={`사용 ${used.toLocaleString()}장, 한도 ${limit.toLocaleString()}장, 누적토큰 ${totalTokens.toLocaleString()}`}
      title={`사용 ${used.toLocaleString()}장 / 한도 ${limit.toLocaleString()}장 / 누적토큰 ${totalTokens.toLocaleString()}`}
    >
      ...
      <span className="topbarQuotaSlash" aria-hidden="true">/</span>
      <span className="topbarQuotaItem tokens"><em>누적토큰</em><strong>{formatCompactCount(totalTokens)}</strong></span>
    </div>
  );
}
...
function AdminApp({ token, onLogout }) {
  ...
  return <main className="adminPage">
    ...
    <div className="adminContent">
      ...
      <section className="adminTableWrap"><table className="adminTable"><thead><tr><th>아이디</th><th>총 한도</th><th>사용</th><th>남음</th><th>누적토큰</th><th>상태</th><th>관리</th></tr></thead><tbody>
        {accounts.map(a=><tr key={a.id}><td><b>{a.login_id}</b></td><td>{a.limit_count}</td><td>{a.used_count}</td><td>{Math.max(0,a.limit_count-a.used_count)}</td><td title={`${Number(a.total_tokens || 0).toLocaleString()} tokens`}>{formatCompactCount(a.total_tokens)}</td><td>{a.active?'사용중':'중지'}</td><td><div className="adminButtons">
