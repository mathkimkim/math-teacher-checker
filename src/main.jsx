...
function CheckerApp({ auth, onLogout, onAccountUpdate }) {
  ...
  const [schoolLevel, setSchoolLevel] = useState('elementary-middle');
  ...
  async function analyzeOne(item) {
    ...
    const res = await fetch('/.netlify/functions/analyze', {
      method: 'POST',
      headers: apiHeaders(token),
      body: JSON.stringify({ imageDataUrl, fileName: item.name, schoolLevel })
    });
  ...
  async function analyzeAll() {
    ...
    for (const item of items) {
      try {
        ...
        setItems((prev) => prev.map((x) => x.id === item.id ? { ...x, status: 'done', result, error: null, analysisLevel: schoolLevel } : x));
      } catch (error) {
  ...
  return (
    <main className="appShell">
      ...
      <div className="appContent">
        ...
        <section className="schoolLevelSelector" aria-label="분석 모델 선택">
          <button type="button" className={schoolLevel === 'elementary-middle' ? 'active' : ''} disabled={busy} onClick={() => setSchoolLevel('elementary-middle')}>초·중등</button>
          <button type="button" className={schoolLevel === 'high' ? 'active' : ''} disabled={busy} onClick={() => setSchoolLevel('high')}>고등</button>
        </section>
...
function AdminApp({ token, onLogout }) {
  ...
  async function downloadExcel() {
    ...
    const summary = accounts.map((account) => {
      const flash = modelUsage(account.id, 'gemini-3.5-flash');
      ...
      return {
        ...
        'Flash 입력토큰': flash.input, 'Flash 일반출력': flash.answer, 'Flash 추론': flash.thinking, 'Flash 총토큰': flash.total, 'Flash 비용(USD)': flash.cost,
        'Pro 입력토큰': pro.input, 'Pro 일반출력': pro.answer, 'Pro 추론': pro.thinking, 'Pro 총토큰': pro.total, 'Pro 비용(USD)': pro.cost,
        '전체 누적토큰': account.total_tokens || 0, '총비용(USD)': flash.cost + pro.cost, 생성일: account.created_at
      };
    });
  ...
  return <main className="adminPage">
    ...
    <div className="adminContent">
      <section className="adminUsageSummary" aria-label="전체 API 사용 현황">
        ...
        <p>Gemini 3.5 Flash 및 Gemini 3.1 Pro Preview 실제 사용량 기준</p>
      </section>
      ...
      <section className="adminTableWrap"><table className="adminTable"><thead><tr><th>아이디</th><th>총 한도</th><th>사용</th><th>남음</th><th>3.5 Flash 토큰·비용</th><th>3.1 Pro 토큰·비용</th><th>누적토큰</th><th>총비용(USD)</th><th>상태</th><th>관리</th></tr></thead><tbody>
        {accounts.map(a=>{
          const flash = modelUsage(a.id, 'gemini-3.5-flash'); const pro = modelUsage(a.id, 'gemini-3.1-pro-preview');
          return <tr key={a.id}><td><b>{a.login_id}</b></td><td>{a.limit_count}</td><td>{a.used_count}</td><td>{Math.max(0,a.limit_count-a.used_count)}</td><td title={`입력 ${flash.input.toLocaleString()} / 일반출력 ${flash.answer.toLocaleString()} / 추론 ${flash.thinking.toLocaleString()}`}><span>입 {formatCompactCount(flash.input)} · 출 {formatCompactCount(flash.answer)} · 추 {formatCompactCount(flash.thinking)}</span><br/><b>총 {formatCompactCount(flash.total)} · {formatUsd(flash.cost)}</b></td><td title={`입력 ${pro.input.toLocaleString()} / 일반출력 ${pro.answer.toLocaleString()} / 추론 ${pro.thinking.toLocaleString()}`}><span>입 {formatCompactCount(pro.input)} · 출 {formatCompactCount(pro.answer)} · 추 {formatCompactCount(pro.thinking)}</span><br/><b>총 {formatCompactCount(pro.total)} · {formatUsd(pro.cost)}</b></td><td title={`${Number(a.total_tokens || 0).toLocaleString()} total tokens`}>{formatCompactCount(a.total_tokens)}</td><td><b>{formatUsd(flash.cost + pro.cost)}</b></td><td>{a.active?'사용중':'중지'}</td><td><div className="adminButtons">
