...
function VerdictDialog({ current, onSelect, onClose }) {
  useEffect(() => {
    ...
    <div className="verdictDialogBackdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <div className="verdictDialog" role="dialog" aria-modal="true" aria-label="판정 변경">
        ...
        <div className="verdictChoices">
          <button type="button" className={current === '맞음' ? 'correct selected' : 'correct'} onClick={() => onSelect('맞음')}>
            <span aria-hidden="true">✓</span> 맞음
          </button>
          <button type="button" className={current === '틀림' ? 'incorrect selected' : 'incorrect'} onClick={() => onSelect('틀림')}>
...
function ResultCard({ item, onCopy, onChangeVerdict }) {
  ...
  return (
    <div className="result">
      {problems.map((problem, index) => {
        ...
        const showExpression = verdict === '틀림' && canShowOriginalError && expression;
          ...
          <div className="resultBlock" key={`${number}-${index}`}>
            <div className="resultHeading">
              ...
              <div className="verdictControls">
                ...
                <button type="button" className="verdictEditButton" onClick={() => onChangeVerdict(index, verdict)} aria-label={`${number}번 판정 변경`} title="판정 변경">
...
function CheckerApp({ auth, onLogout, onAccountUpdate }) {
  ...
  function openVerdictDialog(itemId, problemIndex, current) {
    setVerdictTarget({ itemId, problemIndex, current });
  }

  function changeVerdict(nextVerdict) {
  ...
  function copyResult(item) {
    ...
    const lines = problems.map((problem, index) => {
      ...
      const originalVerdict = problem?.originalVerdict || verdict;
      const canShowOriginalError = !problem?.manuallyChanged || originalVerdict === '틀림';
      if (verdict === '틀림' && canShowOriginalError && expression) details.push(expression);
  ...
  const analyzedProblems = items.reduce((sum, item) => sum + (Array.isArray(item.result?.problems) ? item.result.problems.length : 0), 0);
  const incorrectProblems = items.reduce((sum, item) => sum + (Array.isArray(item.result?.problems)
    ? item.result.problems.filter((problem) => problem?.verdict === '틀림').length
    : 0), 0);
      ...
      <div className="appContent">
        <section className="quickStartGrid" aria-label="빠른 시작">
          ...
          >
            ...
            <p className="miniUploadHint">최대 10장까지 선택 가능</p>
          </article>
        </section>

        <section className="analysisCounter" aria-live="polite">
          <div><span>총 분석 문제</span><strong>{analyzedProblems.toLocaleString()}개</strong></div>
          <div className="incorrect"><span>틀린 문제</span><strong>{incorrectProblems.toLocaleString()}개</strong></div>
        </section>
            ...
            <article className="card" key={item.id}>
              ...
              <div className="cardBody">
                ...
                {item.result && <ResultCard
                  ...
                  onChangeVerdict={(problemIndex, current) => openVerdictDialog(item.id, problemIndex, current)}
                />}
              </div>
            </article>
          ))}
...
function AdminApp({ token, onLogout }) {
  ...
  const proUsage = usage.filter((row) => row.model === 'gemini-3.1-pro-preview');
  const totalInputTokens = proUsage.reduce((sum, row) => sum + Number(row.input_tokens || 0), 0);
  const totalAnswerTokens = proUsage.reduce((sum, row) => sum + Number(row.answer_tokens || 0), 0);
  const totalThinkingTokens = proUsage.reduce((sum, row) => sum + Number(row.thinking_tokens || 0), 0);
  const totalEstimatedCost = proUsage.reduce((sum, row) => sum + Number(row.estimated_cost_usd || 0), 0);
  ...
  return <main className="adminPage">
    ...
    <div className="adminContent">
      <section className="adminUsageSummary" aria-label="전체 API 사용 현황">
        ...
        <p>Gemini 3.1 Pro Preview 실제 사용량 기준</p>
      </section>
      ...
      <section className="adminTableWrap"><table className="adminTable"><thead><tr><th>아이디</th><th>총 한도</th><th>사용</th><th>남음</th><th>3.1 Pro 토큰·비용</th><th>총비용(USD)</th><th>상태</th><th>관리</th></tr></thead><tbody>
        {accounts.map(a=>{
          const pro = modelUsage(a.id, 'gemini-3.1-pro-preview');
          return <tr key={a.id}><td><b>{a.login_id}</b></td><td>{a.limit_count}</td><td>{a.used_count}</td><td>{Math.max(0,a.limit_count-a.used_count)}</td><td title={`입력 ${pro.input.toLocaleString()} / 일반출력 ${pro.answer.toLocaleString()} / 추론 ${pro.thinking.toLocaleString()}`}><span>입 {formatCompactCount(pro.input)} · 출 {formatCompactCount(pro.answer)} · 추 {formatCompactCount(pro.thinking)}</span><br/><b>총 {formatCompactCount(pro.total)} · {formatUsd(pro.cost)}</b></td><td><b>{formatUsd(pro.cost)}</b></td><td>{a.active?'사용중':'중지'}</td><td><div className="adminButtons">
